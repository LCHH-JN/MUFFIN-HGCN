...
class InstanceNormalization(nn.Module):
    ...
    def forward(self, x):
        ...
        return F.batch_norm(x, mean, variance, None, None, training=self.training, eps=1e-12)

class MultiLayerFC(nn.Module):
    def __init__(self, input_dim, hidden_dims, output_dim, normalization_type='fused_BN_center', activation_type='ReLU'):
        ...
        for h_dim in hidden_dims:
            ...
            layers.append(get_activation_fn(activation_type)())
            in_dim = h_dim
...
def get_activation_fn(name):
    ...
    return activation_dict.get(name, nn.Identity)
...
class ClassAwarePredictor(nn.Module):
    ...
    def forward(self, features, num_classes, box_encoding_len):
        logits = self.cls_fn(features)
        box_encodings_list = []

        for class_idx in range(num_classes):
            box_encodings = self.loc_fn(features)
            box_encodings_list.append(box_encodings.unsqueeze(1))  # Add dimension for class index

        box_encodings = torch.cat(box_encodings_list, dim=1)
        return logits, box_encodings

class GraphNetAutoCenter(nn.Module):
    def __init__(self, edge_feature_fn, aggregation_fn, update_fn):
        super(GraphNetAutoCenter, self).__init__()
        self.edge_feature_fn = edge_feature_fn
        self.aggregation_fn = aggregation_fn
        self.update_fn = update_fn

    def forward(self, input_vertex_features, edges):
        s_vertex_features = input_vertex_features[edges[:, 0]]
        d_vertex_features = input_vertex_features[edges[:, 1]]

        edge_features = torch.cat([s_vertex_features, d_vertex_features - s_vertex_features], dim=-1)
        edge_features = self.edge_feature_fn(edge_features)

        # Aggregate features (you can use max, mean, etc. based on your need)
        aggregated_edge_features = pyg_nn.global_max_pool(edge_features, edges[:, 1])

        # Update vertex features
        updated_features = self.update_fn(aggregated_edge_features)
        output_vertex_features = updated_features + input_vertex_features
...
class HGCN(nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        x = self.conv4(x, edge_index)  # 通常最后一层不加激活函数
        return x
...
class TransFusionHead(nn.Module):
    ...
    def forward_single(self, inputs, img_inputs, img_metas):
        ...
        if self.fuse_img:
            ...
            # 1. 图的构建
            # 假设 bev_feat 和 lidar_feat 的形状分别为 [BS, C, H, W]，[BS, C_lidar, N_lidar]
            batch_size, num_channels, height, width = bev_feat.shape
            num_nodes = height * width  # 每个像素都是一个节点
            lidar_feat_flatten = lidar_feat.view(batch_size, lidar_feat.shape[1], -1)  # [BS, C_lidar, N_lidar]

            # 独立创建图节点
            # 将 bev_feat 展平为 [BS, C, num_nodes]
            bev_feat_flatten = bev_feat.view(batch_size, num_channels, -1).permute(0, 2, 1)  # [BS, num_nodes, C]

            # 2. 创建相似度矩阵（升维以计算相似度）
            similarity_matrix = torch.cdist(bev_feat_flatten, lidar_feat_flatten.permute(0, 2, 1),
                                            p=2)  # [BS, num_nodes, N_lidar]
            similarity_threshold = 0.5  # 设置阈值
            edge_index = []
            for b in range(batch_size):
                for i in range(num_nodes):
                    for j in range(lidar_feat_flatten.shape[2]):
                        if similarity_matrix[b, i, j] < similarity_threshold:
                            edge_index.append((b * num_nodes + i, b * (lidar_feat_flatten.shape[2]) + j))  # 添加边
            edge_index = torch.tensor(edge_index, dtype=torch.long).t().contiguous()  # 转置并转为长整型

            # 3. 实例化和运行 GNN
            gnn = SimpleGNN(input_dim=num_channels, hidden_dim=128, output_dim=num_channels)  # 假定输入输出通道与原有相同
            # 4. 前向传播
            bev_feat_gnn = gnn(bev_feat_flatten.view(-1, num_channels), edge_index)  # [num_nodes * BS, C]
            bev_feat_gnn = bev_feat_gnn.view(batch_size, num_nodes, -1)  # 重新调整为 [BS, num_nodes, C]

            # 5. 更新 bev_feat
            bev_feat = bev_feat_gnn.permute(0, 2, 1).view(batch_size, num_channels, height, width)  # [BS, C, H, W] 格式
