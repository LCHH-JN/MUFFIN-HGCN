...
class TransFusionHead(nn.Module):
    ...
    def forward_single(self, inputs, img_inputs, img_metas):
        ...
        if self.fuse_img:
            ...
            bev_feat = bev_feat.view(batch_size, num_channels, assumed_height, assumed_width)
            
            batch_size, num_channels, height, width = bev_feat.shape
            num_nodes = height * width  # 每个像素作为节点
            ...
            gnn = SimpleGraphNN(input_dim=num_channels, hidden_dim=128, output_dim=num_channels)  # 假设输入输出通道相同
            ...
            bev_feat = bev_feat_gnn.permute(0, 2, 1).view(batch_size, num_channels, height, width)  # [BS, C, H, W]
